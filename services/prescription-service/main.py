"""
Prescription Service FastAPI Application.
...
from database import close_db, get_database, init_db
...
REQUEST_COUNT = Counter(
    "prescription_service_requests_total",
    "Total HTTP requests",
    ["method", "endpoint", "status"],
)

REQUEST_LATENCY = Histogram(
    "prescription_service_request_duration_seconds",
...
async def lifespan(_app: FastAPI):
    ...
    """
    logger.info("prescription_service_starting")
    ...
    # Cleanup
    close_db()
    logger.info("prescription_service_shutting_down")


# Initialize FastAPI application
app = FastAPI(
    title="Prescription Service",
    description="Healthcare Patient Management System - Prescription/Clinical Microservice",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc",
    openapi_url="/openapi.json",
    root_path="/api/v1/prescription",
    lifespan=lifespan,
)
...
# Include API Router
app.include_router(api.router, prefix="/api/v1", tags=["Prescription Service"])
